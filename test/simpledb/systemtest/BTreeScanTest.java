...
/**
 * Dumps the contents of a table. args[1] is the number of columns.  E.g., if it's 5, then
 * BTreeScanTest will end up dumping the contents of f4.0.txt.
 */
public class BTreeScanTest extends SimpleDbTestBase {
  ...
  public void testRewindPredicates() throws IOException, DbException, TransactionAbortedException {
    ...
    IndexPredicate ipred = new IndexPredicate(Op.EQUALS,
        new IntField(r.nextInt(BTreeUtility.MAX_RAND_VALUE)));
    ...
    // GREATER_THAN
    tuplesFiltered.clear();
    ipred = new IndexPredicate(Op.GREATER_THAN_OR_EQ,
        new IntField(r.nextInt(BTreeUtility.MAX_RAND_VALUE)));
    it = tuples.iterator();
    while (it.hasNext()) {
      List<Integer> tup = it.next();
