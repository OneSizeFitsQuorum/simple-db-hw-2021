...
public class TestUtil {
   ...
   * @return a OpIterator over a list of tuples constructed over the data provided in the
   * constructor. This iterator is already open.
   ...
   * @param width   the number of fields in each tuple
   * @param tupdata an array such that the ith element the jth tuple lives in slot j * width + i.
   *                Objects can be strings or ints;  tuples must all be of same type.
   * @return a OpIterator over a list of tuples constructed over the data provided in the
   * constructor. This iterator is already open.
   * @require tupdata.length % width == 0
   */
  ...
  /**
   * @return true iff the tuples have the same number of fields and corresponding fields in the two
   * Tuples are all equal.
   */
  ...
  /**
   * Check to see if the DbIterators have the same number of tuples and each tuple pair in parallel
   * iteration satisfies compareTuples . If not, throw an assertion.
   */
  ...
  /**
   * Check to see if every tuple in expected matches <b>some</b> tuple in actual via compareTuples.
   * Note that actual may be a superset. If not, throw an assertion.
   */
  ...
  public static class MockScan implements OpIterator {
    ...
    /**
     * Creates a fake SeqScan that returns tuples sequentially with 'width' fields, each with the
     * same value, that increases from low (inclusive) and high (exclusive) over getNext calls.
     */
