...
public interface DbFile {
   ...
   *
   * @param p The page to write.  page.getId().pageno() specifies the offset into the file where the
   ...
   * BufferPool#getPage}, rather than {@link #readPage} to iterate through the pages.
   *
   ...
   * the table via {@link Catalog#getDatabaseFile} and {@link Catalog#getTupleDesc}.
   * <p>
   * Implementation note:  you will need to generate this tableid somewhere, ensure that each
   * HeapFile has a "unique id," and that you always return the same value for a particular
   * HeapFile. A simple implementation is to use the hash code of the absolute path of the file
   * underlying the HeapFile, i.e. <code>f.getAbsoluteFile().hashCode()</code>.
   *
