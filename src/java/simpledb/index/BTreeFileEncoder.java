...
/**
 * BTreeFileEncoder reads a comma delimited text file and converts it to pages of binary data in the
 * appropriate format for simpledb B+ tree pages.
 */
...
public class BTreeFileEncoder {
   ...
   * @param hFile          - the data file for the HeapFile to be used as an intermediate conversion
   *                       step
  ...
  public static BTreeFile convert(File inFile, File hFile, File bFile, int npagebytes,
    ...
    int nrecords =
        (npagebytes * 8 - leafpointerbytes * 8) / (nrecbytes * 8 + 1);  //floor comes for free
    ...
    int nentries = (npagebytes * 8 - internalpointerbytes * 8 - 1) / (nentrybytes * 8
        + 1);  //floor comes for free
    ...
    if (page2.size() == 0) {
      ...
      byte[] secondToLastPageBytes = convertToLeafPage(secondToLastPg, npagebytes, numFields,
          typeAr, keyField);
      BTreePageId secondToLastPid = new BTreePageId(tableid, bf.numPages() + 1, BTreePageId.LEAF);
      BTreeLeafPage secondToLastPage = new BTreeLeafPage(secondToLastPid, secondToLastPageBytes,
          keyField);
      ...
      // update the parent by "copying up" the next key
      BTreeEntry copyUpEntry = new BTreeEntry(lastPg.get(0).getField(keyField), secondToLastPid,
          lastPid);
      updateEntries(entries, bf, copyUpEntry, 0, nentries, npagebytes,
          keyType, tableid, keyField);
    }
    ...
    for (int i = 0; i < entries.size(); i++) {
      ...
      if (size <= nentries) {
        ...
        byte[] secondToLastPageBytes = convertToInternalPage(secondToLastPg, npagebytes, keyType,
            childPageCategory);
        BTreePageId secondToLastPid = new BTreePageId(tableid, bf.numPages() + 1,
            BTreePageId.INTERNAL);
        bf.writePage(new BTreeInternalPage(secondToLastPid, secondToLastPageBytes, keyField));

        byte[] lastPageBytes = convertToInternalPage(lastPg, npagebytes, keyType,
            childPageCategory);
        ...
        BTreeEntry pushUpEntry = new BTreeEntry(entries.get(i).get(size / 2).getKey(),
  ...
  private static void updateEntries(List<List<BTreeEntry>> entries,
    ...
    if (size > 0) {
      ...
      entries.get(level)
          .set(size - 1, new BTreeEntry(prev.getKey(), prev.getLeftChild(), e.getLeftChild()));
      if (size == nentries * 2 + 1) {
        // write out a page of entries
        ArrayList<BTreeEntry> pageEntries = new ArrayList<>(
            entries.get(level).subList(0, nentries));
        byte[] internalPageBytes = convertToInternalPage(pageEntries, npagebytes, keyType,
            childPageCategory);
        BTreePageId internalPid = new BTreePageId(tableid, bf.numPages() + 1, BTreePageId.INTERNAL);
        bf.writePage(new BTreeInternalPage(internalPid, internalPageBytes, keyField));

        // update the parent by "pushing up" the next key
        BTreeEntry pushUpEntry = new BTreeEntry(entries.get(level).get(nentries).getKey(),
            internalPid, null);
        updateEntries(entries, bf, pushUpEntry, level + 1, nentries, npagebytes,
            keyType, tableid, keyField);
        List<BTreeEntry> remainingEntries = new ArrayList<>(
            entries.get(level).subList(nentries + 1, size));
  ...
  public static byte[] convertToLeafPage(List<Tuple> tuples, int npagebytes,
    ...
    int nrecords =
        (npagebytes * 8 - pointerbytes * 8) / (nrecbytes * 8 + 1);  //floor comes for free
  ...
  public static byte[] convertToInternalPage(List<BTreeEntry> entries, int npagebytes,
    ...
    int nentries =
        (npagebytes * 8 - pointerbytes * 8 - 1) / (nentrybytes * 8 + 1);  //floor comes for free
