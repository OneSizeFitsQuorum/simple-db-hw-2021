...
/**
 * Each instance of BTreeHeaderPage stores data for one page of a BTreeFile and implements the Page
...
public class BTreeHeaderPage implements Page {
  ...
  /**
   * Create a BTreeHeaderPage from a set of bytes of data read from disk. The format of a
   * BTreeHeaderPage is two pointers to the next and previous header pages, followed by a set of
   * bytes indicating which pages in the file are used or available
   *
   ...
   * getPageData to the BTreeHeaderPage constructor and have it produce an identical BTreeHeaderPage
   * object.
   *
  ...
  /**
   * Static method to generate a byte array corresponding to an empty BTreeHeaderPage. Used to add
   * new, empty pages to the file. Passing the results of this method to the BTreeHeaderPage
   * constructor will create a BTreeHeaderPage with no valid data in it.
   *
