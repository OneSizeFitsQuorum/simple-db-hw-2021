...
public class BTreeUtility {
  ...
  /**
   * @return a Tuple with a single IntField with value n and with RecordId(BTreePageId(1,2,
   * BTreePageId.LEAF), 3)
   */
  ...
  /**
   * @return a Tuple with an IntField for every element of tupdata and RecordId(BTreePageId(1, 2,
   * BTreePageId.LEAF), 3)
   */
  ...
  /**
   * @return a Tuple with a 'width' IntFields each with value n and with RecordId(BTreePageId(1, 2,
   * BTreePageId.LEAF), 3)
   */
  ...
  /**
   * @return a BTreeEntry with an IntField with value n and with RecordId(BTreePageId(1,2,
  ...
  public static BTreeLeafPage createRandomLeafPage(BTreePageId pid, int columns, int keyField,
    ...
    Type[] typeAr = new Type[columns];
    Arrays.fill(typeAr, Type.INT_TYPE);
    byte[] data = BTreeFileEncoder
        .convertToLeafPage(BTreeUtility.generateRandomTuples(columns, numTuples, min, max),
  ...
  public static BTreeInternalPage createRandomInternalPage(BTreePageId pid, int keyField,
    ...
    byte[] data = BTreeFileEncoder.convertToInternalPage(BTreeUtility
            .generateRandomEntries(numKeys, pid.getTableId(), childPageCategory, minKey, maxKey,
                minChildPtr),
  ...
  /**
   * A utility method to create a new BTreeFile with no data, assuming the path does not already
   * exist. If the path exists, the file will be overwritten. The new table will be added to the
  ...
  /**
   * A utility method to create a new BTreeFile with no data, with the specified number of pages,
   * assuming the path does not already exist. If the path exists, the file will be overwritten. The
   * new table will be added to the Catalog with the specified number of columns as IntFields
   * indexed on the keyField.
   */
   ...
   *
   * @return a handle to the Thread that will attempt to search for tuple(s) after it has been
   * started
   */
  static class BTreeReader extends Thread {
    ...
    /**
     * @return an Exception instance if one occurred while searching for the tuple(s); null
     * otherwise
     */
    public Exception getError() {
      synchronized (elock) {
