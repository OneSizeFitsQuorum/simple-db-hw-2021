...
public class BTreeScan implements OpIterator {
   ...
   * @param ipred      The index predicate to match. If null, the scan will return all tuples in
   *                   sorted order
   */
  ...
  public void reset(int tableid, String tableAlias) {
    ...
    if (ipred == null) {
      ...
      this.it = ((BTreeFile) Database.getCatalog().getDatabaseFile(tableid))
          .indexIterator(tid, ipred);
    }
  ...
  /**
   * Returns the TupleDesc with field names from the underlying BTreeFile, prefixed with the
   ...
   *
   * @return the TupleDesc with field names from the underlying BTreeFile, prefixed with the
