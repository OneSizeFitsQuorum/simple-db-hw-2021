...
/**
 * Each instance of BTreeLeafPage stores data for one page of a BTreeFile and implements the Page
...
public class BTreeLeafPage extends BTreePage {
  ...
  public void checkRep(int fieldid, Field lowerBound, Field upperBound, boolean checkoccupancy,
    ...
    assert null == upperBound || null == prev || (prev
        .compare(Predicate.Op.LESS_THAN_OR_EQ, upperBound));

    assert !checkoccupancy || depth <= 0 || (getNumTuples() >= getMaxTuples() / 2);
  }

  /**
   * Create a BTreeLeafPage from a set of bytes of data read from disk. The format of a
   * BTreeLeafPage is a set of header bytes indicating the slots of the page that are in use, and
   * some number of tuple slots, as well as some extra bytes for the parent and sibling pointers.
   * Specifically, the number of tuples is equal to: <p> floor((BufferPool.getPageSize()*8 - extra
   * bytes*8) / (tuple size * 8 + 1))
  ...
  /**
   * Computes the number of bytes in the header of a page in a BTreeFile with each tuple occupying
   * tupleSize bytes
   */
   ...
   * getPageData to the BTreeLeafPage constructor and have it produce an identical BTreeLeafPage
   * object.
   *
  ...
  public byte[] getPageData() {
    ...
    int zerolen = BufferPool.getPageSize() - (header.length + td.getSize() * tuples.length
        + 3 * INDEX_SIZE); //- numSlots * td.getSize();
  ...
  /**
   * Adds the specified tuple to the page such that all records remain in sorted order; the tuple
   * should be updated to reflect that it is now stored on this page.
   *
  ...
  /**
   * @return a reverse iterator over all tuples on this page (calling remove on this iterator throws
   * an UnsupportedOperationException) (note that this iterator shouldn't return tuples in empty
   * slots!)
   */
