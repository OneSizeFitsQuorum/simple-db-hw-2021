...
/**
 * BTreeFile is an implementation of a DbFile that stores a B+ tree. Specifically, it stores a
 * pointer to a root page, a set of internal pages, and a set of leaf pages, which contain a
 * collection of tuples in sorted order. BTreeFile works closely with BTreeLeafPage,
...
public class BTreeFile implements DbFile {
  ...
  /**
   * Returns an ID uniquely identifying this BTreeFile. Implementation note: you will need to
   * generate this tableid somewhere and ensure that each BTreeFile has a "unique id," and that you
   * always return the same value for a particular BTreeFile. We suggest hashing the absolute file
   * name of the file underlying the BTreeFile, i.e. f.getAbsoluteFile().hashCode().
   *
  ...
  /**
   * Read a page from the file on disk. This should not be called directly but should be called from
   * the BufferPool via getPage()
   *
  ...
  public Page readPage(PageId pid) {
    ...
    try (BufferedInputStream bis = new BufferedInputStream(new FileInputStream(f))) {
      if (id.pgcateg() == BTreePageId.ROOT_PTR) {
        ...
        if (bis.skip(BTreeRootPtrPage.getPageSize() + (long) (id.getPageNumber() - 1) * BufferPool
            .getPageSize()) !=
            BTreeRootPtrPage.getPageSize() + (long) (id.getPageNumber() - 1) * BufferPool
                .getPageSize()) {
  ...
  /**
   * Write a page to disk.  This should not be called directly but should be called from the
   * BufferPool when pages are flushed to disk
   *
  ...
  public void writePage(Page page) throws IOException {
    ...
    if (id.pgcateg() == BTreePageId.ROOT_PTR) {
      ...
      rf.seek(
          BTreeRootPtrPage.getPageSize() + (long) (page.getId().getPageNumber() - 1) * BufferPool
              .getPageSize());
      rf.write(data);
      rf.close();
    }
  ...
  /**
   * Recursive function which finds and locks the leaf page in the B+ tree corresponding to the
   * left-most page possibly containing the key field f. It locks all internal nodes along the path
   * to the leaf node with READ_ONLY permission, and locks the leaf node with permission perm.
   * <p>
  ...
  /**
   * Convenience method to find a leaf page when there is no dirtypages HashMap. Used by the
   * BTreeFile iterator.
   *
   * @param tid - the transaction id
   * @param pid - the current page being searched
  ...
  /**
   * Split a leaf page to make room for new tuples and recursively split the parent node as needed
   * to accommodate a new entry. The new entry should have a key matching the key field of the first
   * tuple in the right-hand page (the key is "copied up"), and child pointers pointing to the two
   * leaf pages resulting from the split.  Update sibling pointers and parent pointers as needed.
   * <p>
   ...
   * @param field      - the key field of the tuple to be inserted after the split is complete.
   *                   Necessary to know which of the two pages to return.
  ...
  /**
   * Split an internal page to make room for new entries and recursively split its parent page as
   * needed to accommodate a new entry. The new entry for the parent should have a key matching the
   * middle key in the original internal page being split (this key is "pushed up" to the parent).
   ...
   * @param field      - the key field of the entry to be inserted after the split is complete.
   *                   Necessary to know which of the two pages to return.
  ...
  /**
   * Method to encapsulate the process of getting a parent page ready to accept new entries. This
   * may mean creating a page to become the new root of the tree, splitting the existing parent page
   * if there are no empty slots, or simply locking and returning the existing parent page.
   *
   * @param tid        - the transaction id
   * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
   * @param parentId   - the id of the parent. May be an internal page or the RootPtr page
   * @param field      - the key of the entry which will be inserted. Needed in case the parent must
   *                   be split to accommodate the new entry
  ...
  private BTreeInternalPage getParentWithEmptySlots(TransactionId tid, Map<PageId, Page> dirtypages,
    ...
    if (parentId.pgcateg() == BTreePageId.ROOT_PTR) {
      ...
      BTreePage prevRootPage = (BTreePage) getPage(tid, dirtypages, prevRootId,
  ...
  /**
   * Update the parent pointer of every child of the given page so that it correctly points to the
   * parent
   *
   * @param tid        - the transaction id
   * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
  ...
  /**
   * Method to encapsulate the process of locking/fetching a page.  First the method checks the
   * local cache ("dirtypages"), and if it can't find the requested page there, it fetches it from
   * the buffer pool. It also adds pages to the dirtypages cache if they are fetched with read-write
   * permission, since presumably they will soon be dirtied by this transaction.
   * <p>
   * This method is needed to ensure that page updates are not lost if the same pages are accessed
   * multiple times.
   *
   * @param tid        - the transaction id
   * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
  ...
  /**
   * Insert a tuple into this BTreeFile, keeping the tuples in sorted order. May cause pages to
   * split if the page where tuple t belongs is full.
   *
   * @param tid - the transaction id
   * @param t   - the tuple to insert
   * @return a list of all pages that were dirtied by this operation. Could include many pages since
   * parent pointers will need to be updated when an internal node splits.
  ...
  public List<Page> insertTuple(TransactionId tid, Tuple t)
    ...
    if (rootId
        == null) { // the root has just been created, so set the root pointer to point to it
    ...
    BTreeLeafPage leafPage = findLeafPage(tid, dirtypages, rootId, Permissions.READ_WRITE,
        t.getField(keyField));
  ...
  /**
   * Handle the case when a B+ tree page becomes less than half full due to deletions. If one of its
   * siblings has extra tuples/entries, redistribute those tuples/entries. Otherwise merge with one
   * of the siblings. Update pointers as needed.
   *
   * @param tid        - the transaction id
   * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
   ...
   * @throws IOException
   * @throws TransactionAbortedException
   * @see #handleMinOccupancyLeafPage(TransactionId, Map, BTreeLeafPage, BTreeInternalPage,
   * BTreeEntry, BTreeEntry)
   * @see #handleMinOccupancyInternalPage(TransactionId, Map, BTreeInternalPage, BTreeInternalPage,
   * BTreeEntry, BTreeEntry)
   */
  private void handleMinOccupancyPage(TransactionId tid, Map<PageId, Page> dirtypages,
      BTreePage page)
      throws DbException, IOException, TransactionAbortedException {
    ...
    if (page.getId().pgcateg() == BTreePageId.LEAF) {
      handleMinOccupancyLeafPage(tid, dirtypages, (BTreeLeafPage) page, parent, leftEntry,
          rightEntry);
    } else { // BTreePageId.INTERNAL
      handleMinOccupancyInternalPage(tid, dirtypages, (BTreeInternalPage) page, parent, leftEntry,
          rightEntry);
    }
  }

  /**
   * Handle the case when a leaf page becomes less than half full due to deletions. If one of its
   * siblings has extra tuples, redistribute those tuples. Otherwise merge with one of the siblings.
   ...
   * @throws IOException
   * @throws TransactionAbortedException
   * @see #mergeLeafPages(TransactionId, Map, BTreeLeafPage, BTreeLeafPage, BTreeInternalPage,
   * BTreeEntry)
   * @see #stealFromLeafPage(BTreeLeafPage, BTreeLeafPage, BTreeInternalPage, BTreeEntry, boolean)
   */
  private void handleMinOccupancyLeafPage(TransactionId tid, Map<PageId, Page> dirtypages,
      BTreeLeafPage page,
    ...
    if (leftSiblingId != null) {
      BTreeLeafPage leftSibling = (BTreeLeafPage) getPage(tid, dirtypages, leftSiblingId,
          Permissions.READ_WRITE);
   ...
   * half full.  Update the parent's entry so that the key matches the key field of the first tuple
   * in the right-hand page.
   *
   * @param page           - the leaf page which is less than half full
  ...
  /**
   * Handle the case when an internal page becomes less than half full due to deletions. If one of
   * its siblings has extra entries, redistribute those entries. Otherwise merge with one of the
   * siblings. Update pointers as needed.
   *
   * @param tid        - the transaction id
   * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
   ...
   * @throws IOException
   * @throws TransactionAbortedException
   * @see #mergeInternalPages(TransactionId, Map, BTreeInternalPage, BTreeInternalPage,
   * BTreeInternalPage, BTreeEntry)
   * @see #stealFromLeftInternalPage(TransactionId, Map, BTreeInternalPage, BTreeInternalPage,
   * BTreeInternalPage, BTreeEntry)
   * @see #stealFromRightInternalPage(TransactionId, Map, BTreeInternalPage, BTreeInternalPage,
   * BTreeInternalPage, BTreeEntry)
   */
  ...
  /**
   * Steal entries from the left sibling and copy them to the given page so that both pages are at
   * least half full. Keys can be thought of as rotating through the parent entry, so the original
   * key in the parent is "pulled down" to the right-hand page, and the last key in the left-hand
  ...
  /**
   * Steal entries from the right sibling and copy them to the given page so that both pages are at
   * least half full. Keys can be thought of as rotating through the parent entry, so the original
   * key in the parent is "pulled down" to the left-hand page, and the last key in the right-hand
   * page is "pushed up" to the parent.  Update parent pointers as needed.
   *
   * @param tid          - the transaction id
   * @param dirtypages   - the list of dirty pages which should be updated with all new dirty pages
  ...
  /**
   * Merge two leaf pages by moving all tuples from the right page to the left page. Delete the
   * corresponding key and right child pointer from the parent, and recursively handle the case when
   * the parent gets below minimum occupancy. Update sibling pointers as needed, and make the right
  ...
  /**
   * Merge two internal pages by moving all entries from the right page to the left page and
   * "pulling down" the corresponding key from the parent entry. Delete the corresponding key and
   * right child pointer from the parent, and recursively handle the case when the parent gets below
   * minimum occupancy. Update parent pointers as needed, and make the right page available for
   * reuse.
   *
   * @param tid         - the transaction id
   * @param dirtypages  - the list of dirty pages which should be updated with all new dirty pages
   ...
   * below minimum occupancy for non-root internal nodes, it should steal from one of its siblings
  ...
  private void deleteParentEntry(TransactionId tid, Map<PageId, Page> dirtypages,
    ...
    if (parent.getNumEmptySlots() == parent.getMaxEntries()) {
      ...
      BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) getPage(tid, dirtypages, rootPtrId,
          Permissions.READ_WRITE);
  ...
  /**
   * Delete a tuple from this BTreeFile. May cause pages to merge or redistribute entries/tuples if
   * the pages become less than half full.
   *
   * @param tid - the transaction id
   * @param t   - the tuple to delete
   * @return a list of all pages that were dirtied by this operation. Could include many pages since
   * parent pointers will need to be updated when an internal node merges.
   * @see #handleMinOccupancyPage(TransactionId, Map, BTreePage)
   */
  ...
  /**
   * Get a read lock on the root pointer page. Create the root pointer page and root page if
   * necessary.
   *
   * @param tid        - the transaction id
   * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
  ...
  BTreeRootPtrPage getRootPtrPage(TransactionId tid, Map<PageId, Page> dirtypages)
    ...
    return (BTreeRootPtrPage) getPage(tid, dirtypages, BTreeRootPtrPage.getId(tableid),
        Permissions.READ_ONLY);
  }

  /**
   * Get the page number of the first empty page in this BTreeFile. Creates a new page if none of
   * the existing pages are empty.
   *
   * @param tid        - the transaction id
   * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
  ...
  public int getEmptyPageNo(TransactionId tid, Map<PageId, Page> dirtypages)
    ...
    if (headerId != null) {
      BTreeHeaderPage headerPage = (BTreeHeaderPage) getPage(tid, dirtypages, headerId,
          Permissions.READ_ONLY);
  ...
  /**
   * Method to encapsulate the process of creating a new page.  It reuses old pages if possible, and
   * creates a new page if none are available.  It wipes the page on disk and in the cache and
   * returns a clean copy locked with read-write permission
   *
   * @param tid        - the transaction id
   * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
   * @param pgcateg    - the BTreePageId category of the new page.  Either LEAF, INTERNAL, or
   *                   HEADER
  ...
  /**
   * Mark a page in this BTreeFile as empty. Find the corresponding header page (create it if
   * needed), and mark the corresponding slot in the header page as empty.
   *
   * @param tid         - the transaction id
   * @param dirtypages  - the list of dirty pages which should be updated with all new dirty pages
...
//		}
    ...
    if (headerId == null) {
      ...
      BTreeHeaderPage headerPage = (BTreeHeaderPage) getEmptyPage(tid, dirtypages,
          BTreePageId.HEADER);
    ...
    while (headerId != null
        && (headerPageCount + 1) * BTreeHeaderPage.getNumSlots() < emptyPageNo) {
      BTreeHeaderPage headerPage = (BTreeHeaderPage) getPage(tid, dirtypages, headerId,
          Permissions.READ_ONLY);
      prevId = headerId;
      headerId = headerPage.getNextPageId();
    ...
    while ((headerPageCount + 1) * BTreeHeaderPage.getNumSlots() < emptyPageNo) {
      BTreeHeaderPage prevPage = (BTreeHeaderPage) getPage(tid, dirtypages, prevId,
          Permissions.READ_WRITE);
  ...
  /**
   * get the specified tuples from the file based on its IndexPredicate value on behalf of the
   * specified transaction. This method will acquire a read lock on the affected pages of the file,
  ...
  /**
   * Get an iterator for all tuples in this B+ tree file in sorted order. This method will acquire a
   * read lock on the affected pages of the file, and may block until the lock can be acquired.
   *
   * @param tid - the transaction id
...
class BTreeFileIterator extends AbstractDbFileIterator {
  ...
  /**
   * Read the next tuple either from the current page if it has more tuples or from the next page by
   * following the right sibling pointer.
   *
...
class BTreeSearchIterator extends AbstractDbFileIterator {
  ...
  /**
   * Open this iterator by getting an iterator on the first leaf page applicable for the given
   * predicate operation
   */
  public void open() throws DbException, TransactionAbortedException {
    BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) Database.getBufferPool().getPage(
  ...
  /**
   * Read the next tuple either from the current page if it has more tuples matching the predicate
   * or from the next page by following the right sibling pointer.
   *
