...
public class BTreeChecker {
  ...
  /**
   * checks the integrity of the tree: 1) parent pointers. 2) sibling pointers. 3) range invariants.
   * 4) record to page pointers. 5) occupancy invariants. (if enabled)
   */
  ...
  static SubtreeSummary checkSubTree(BTreeFile bt, TransactionId tid, Map<PageId, Page> dirtypages,
    ...
    if (page.getId().pgcateg() == BTreePageId.LEAF) {
      ...
      SubtreeSummary lastRight = checkSubTree(bt, tid, dirtypages, curr.getRightChild(), lowerBound,
          upperBound,
