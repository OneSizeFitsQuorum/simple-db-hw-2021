 ...
 * selecting the best instantiation of a join for a given logical plan.
 */
public class JoinOptimizer {
   ...
   * which plan should be the inner/outer here -- because OpIterator's don't provide any cardinality
   ...
   * <p>
   * The cost of the join should be calculated based on the join algorithm (or algorithms) that you
   * implemented for Lab 2. It should be a function of the amount of data that must be read over the
   * course of the query, as well as the number of CPU opertions performed by your join. Assume that
  ...
  /**
   * Compute a logical, reasonably efficient join on the specified tables. See PS4 for hints on how
   * this should be implemented.
   *
   * @param stats               Statistics for each table involved in the join, referenced by base
   *                            table names, not alias
   ...
   * @param explain             Indicates whether your code should explain its query plan or simply
   *                            execute it
   * @return A List<LogicalJoinNode> that stores joins in the left-deep order in which they should
   * be executed.
   ...
   *
   * @param stats               table stats for all of the tables, referenced by table names rather
   *                            than alias (see {@link #orderJoins})
   ...
   * @param bestCostSoFar       the best way to join joinSet so far (minimum of previous invocations
   *                            of computeCostAndCardOfSubplan for this joinSet, from returned
   *                            CostCard)
   * @param pc                  the PlanCache for this join; should have subplans for all plans of
   *                            size joinSet.size()-1
