...
//            TupleDesc td = s.getTupleDesc();
      ...
      if (plan instanceof Join) {
        ...
        thisNode.text = String.format("%1$s(%2$s),card:%3$d", PROJECT, fields.toString(),
            p.getEstimatedCardinality());
        int upBarShift = parentUpperBarStartShift;
        ...
        thisNode.leftChild = child;
        thisNode.height = currentDepth;
      } else if (plan.getClass().getSuperclass().getSuperclass().getSimpleName()
          .equals("Exchange")) {
        ...
        try {
          ...
          fieldIdx = (Integer) plan.getClass().getMethod("renamedField", (Class<?>[]) null)
              .invoke(plan);
        } catch (Exception e) {
          e.printStackTrace();
        }
        String oldName = plan.getChildren()[0].getTupleDesc().getFieldName(fieldIdx);
        thisNode.text = String.format("%1$s,%2$s->%3$s,card:%4$d", RENAME, oldName, newName,
            plan.getEstimatedCardinality());
        int upBarShift = parentUpperBarStartShift;
