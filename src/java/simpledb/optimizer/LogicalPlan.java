...
/**
 * LogicalPlan represents a logical query plan that has been through the parser and is ready to be
 * processed by the optimizer.
 * <p>
 * A LogicalPlan consits of a collection of table scan nodes, join nodes, filter nodes, a select
 * list, and a group by field. LogicalPlans can only represent queries with one aggregation field
 * and one group by field.
 * <p>
 * LogicalPlans can be converted to physical (optimized) plans using the {@link #physicalPlan}
 * method, which uses the {@link JoinOptimizer} to order joins optimally and to select the best
 * implementations for joins.
 */
...
//    private Query owner;
  ...
  /**
   * Set the text of the query representing this logical plan.  Does NOT parse the specified query
   * -- this method is just used so that the object can print the SQL it represents.
   *
  ...
  /**
   * Given a table alias, return id of the table object (this id can be supplied to {@link
   * Catalog#getDatabaseFile(int)}). Aliases are added as base tables are added via {@link
   * #addScan}.
   *
   ...
   *
   * @param field         The name of the over which the filter applies; this can be a fully
   *                      qualified field (tablename.field or alias.field), or can be a unique field
   *                      name without a tablename qualifier.  If it is an ambiguous name, it will
   *                      throw a ParsingException
   * @param p             The predicate for the filter
   * @param constantValue the constant to compare the predicate against; if field is an integer
   *                      field, this should be a String representing an integer
   * @throws ParsingException if field is not in one of the tables added via {@link #addScan} or if
   *                          field is ambiguous (e.g., two tables contain a field named field.)
   */
   ...
   *
   * @param joinField1 The name of the first join field; this can be a fully qualified name (e.g.,
   *                   tableName.field or alias.field) or may be an unqualified unique field name.
   *                   If the name is ambiguous or unknown, a ParsingException will be thrown.
   * @param joinField2 The name of the second join field
   * @param pred       The join predicate
   * @throws ParsingException if either of the fields is ambiguous, or is not in one of the tables
   *                          added via {@link #addScan}
   */
  ...
  public void addJoin(String joinField1, String joinField2, Predicate.Op pred)
    ...
    LogicalJoinNode lj = new LogicalJoinNode(table1Alias, table2Alias, pureField1, pureField2,
        pred);
   ...
   *                   tableName.field or alias.field) or may be an unqualified unique field name.
   *                   If the name is ambiguous or unknown, a ParsingException will be thrown.
   * @param joinField2 the subquery to join with -- the join field of the subquery is the first
   *                   field in the result set of the query
   * @param pred       The join predicate.
   * @throws ParsingException if either of the fields is ambiguous, or is not in one of the tables
   *                          added via {@link #addScan}
   */
  public void addJoin(String joinField1, OpIterator joinField2, Predicate.Op pred)
      throws ParsingException {
    joinField1 = disambiguateName(joinField1);
  ...
  /**
   * Add a scan to the plan. One scan node needs to be added for each alias of a table accessed by
   * the plan.
   *
   * @param table the id of the table accessed by the plan (can be resolved to a DbFile using {@link
   *              Catalog#getDatabaseFile}
  ...
  /**
   * Add a specified field/aggregate combination to the select list of the query. Fields are output
   * by the query such that the rightmost field is the first added via addProjectField.
   *
  ...
  /**
   * Add an aggregate over the field with the specified grouping to the query.  SimpleDb only
   * supports a single aggregate expression and GROUP BY field.
   *
  ...
  /**
   * Add an ORDER BY expression in the specified order on the specified field.  SimpleDb only
   * supports a single ORDER BY field.
   *
  ...
  /**
   * Given a name of a field, try to figure out what table it belongs to by looking through all of
   * the tables added via {@link #addScan}.
   *
   * @return A fully qualified name of the form tableAlias.name.  If the name parameter is already
   * qualified with a table name, simply returns name.
   * @throws ParsingException if the field cannot be found in any of the tables, or if the field is
   *                          ambiguous (appears in multiple tables)
   */
...
//                int id = 
        ...
        if (tableName == null) {
          ...
          throw new ParsingException("Field " + name
   ...
   * find the optimal plan by using {@link JoinOptimizer#orderJoins} to order the joins in the
   * plan.
   *
   * @param t              The transaction that the returned OpIterator will run as a part of
   * @param baseTableStats a HashMap providing a {@link TableStats} object for each table used in
   *                       the LogicalPlan.  This should have one entry for each table referenced by
   *                       the plan, not one entry for each table alias (so a table t aliases as t1
   *                       and t2 would have just one entry with key 't' in this HashMap).
   * @param explain        flag indicating whether output visualizing the physical query plan should
   *                       be given.
  ...
  public OpIterator physicalPlan(TransactionId t, Map<String, TableStats> baseTableStats,
    ...
    for (LogicalFilterNode lf : filters) {
      ...
      TableStats s = statsMap
          .get(Database.getCatalog().getTableName(this.getTableId(lf.tableAlias)));

      double sel = s
          .estimateSelectivity(subplan.getTupleDesc().fieldNameToIndex(lf.fieldQuantifiedName),
              lf.p, f);
    ...
    for (LogicalJoinNode lj : joins) {
      ...
      if (!isSubqueryJoin) {
        ...
        equivMap
            .put(t2name, t1name);  //keep track of the fact that this new node contains both tables
